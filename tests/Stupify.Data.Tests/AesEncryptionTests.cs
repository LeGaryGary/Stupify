...
namespace Stupify.Data.Tests
{
    public class AesEncryptionTests
    {
        ...
        public void AesTests(string painText, string password)
        {
            var aes = new AesCryptography(password);

            var encrypted = aes.Encrypt(painText);
            var decrypted = aes.Decrypt(encrypted);

            decrypted.Should().Be(painText, "the decrypted string should be unchanged from the original string");
        }
    }
}
